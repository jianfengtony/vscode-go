...
import { GoLanguageClient } from '../goLanguageServer';
...
export namespace TTreeView {
	...
	export var client: GoLanguageClient | undefined;
			...
			.then((result) => {
				...
				client!.protocol2CodeConverter.asReferences(result, token)
					.then(locations => {
						...
						// vscode.commands.executeCommand(`workbench.views.explorer.go.findAllReferences`);
						vscode.commands.executeCommand('workbench.action.focusView', "go.findAllReferences");
